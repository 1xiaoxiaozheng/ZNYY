...
public class EquipRegInfoService_WX {
    ...
    public void syncEquipRegInfo() {
        ...
        try {
            // 获取seeyon中已删除的无形资产信息
            List<AssetRegistrationQueryMapper.DeletedAssetInfo> deletedAssetInfoList = assetRegistrationQueryMapper
                    .selectDeletedAssetInfo();

            if (!deletedAssetInfoList.isEmpty()) {
                // 生成被删除的RID
                List<String> deletedRids = new ArrayList<>();
                for (AssetRegistrationQueryMapper.DeletedAssetInfo deletedInfo : deletedAssetInfoList) {
                ...
                if (!deletedRids.isEmpty()) {
                    ...
                    System.out.println("基于seeyon删除状态标记删除的无形资产RID数量: " + deleteCount);
                    System.out.println("被删除的无形资产RID: " + deletedRids);
                }
            }
        } catch (Exception e) {
