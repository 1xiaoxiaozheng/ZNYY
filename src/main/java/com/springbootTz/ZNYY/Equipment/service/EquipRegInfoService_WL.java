...
public class EquipRegInfoService_WL {
    ...
    public void syncEquipRegInfo() {
        ...
        try {
            // 获取seeyon中已删除的网络信息设备信息
            List<AssetRegistrationNetworkQueryMapper.DeletedNetworkInfo> deletedNetworkInfoList = assetRegistrationNetworkQueryMapper
                    .selectDeletedNetworkInfo();

            if (!deletedNetworkInfoList.isEmpty()) {
                // 生成被删除的RID
                List<String> deletedRids = new ArrayList<>();
                for (AssetRegistrationNetworkQueryMapper.DeletedNetworkInfo deletedInfo : deletedNetworkInfoList) {
                    if (deletedInfo.getField0007() != null && !deletedInfo.getField0007().trim().isEmpty()) {
                        // 获取单位代码（就像正常数据处理一样）
                        String unitCode = String
                                .valueOf(unitInfoToolMapper.selectUnitCodeByName(deletedInfo.getField0006()));
                        String deletedRid = unitCode + "FJZZZYKJYXGS" + deletedInfo.getField0007();
                ...
                if (!deletedRids.isEmpty()) {
                    ...
                    System.out.println("基于seeyon删除状态标记删除的网络信息设备RID数量: " + deleteCount);
                    System.out.println("被删除的网络信息设备RID: " + deletedRids);
                }
            }
        } catch (Exception e) {
