...
public class EquipCardInfoService {
     ...
     * 2. 若 acBvName1 是车辆、房屋土地、家具设备、网络信息设备、医疗专用设备、无形资产且 orgName 为周宁县总医院，用
     * ev_field83<br />
     ...
     * manufactureNo，只有周宁县总医院和周宁医院的医疗专用设备有，ev_field7;否则都是空字符串
    ...
    private EquipCardInfo mapToEquipCardInfo(AssetCardWithDepreciationDTO assetCard) {
        ...
        equipCardInfo.setBuyCost(assetCard.getAcOriMoney());
        equipCardInfo.setNowCost(assetCard.getAcOriMoney());
        equipCardInfo.setBuyDate(formatDate(assetCard.getAcGetDate()));
        ...
        equipCardInfo.setDeprecRate(assetCard.getAcDeprRate());
     ...
     * 折旧年限规则：
     * 1. 若 acBvName1 是家居设备、网络信息设备、医疗专用设备、无形资产、车辆；房屋土地且 orgName 为周宁县医院，用 ev_field81
     * 2. 若 acBvName1 是车辆、房屋土地、家具设备、网络信息设备、医疗专用设备；无形资产且 orgName 为周宁县总医院，用 ev_field83
     * 开始时间优先级：
    ...
    private String getManufacturerName(AssetCardWithDepreciationDTO assetCard, String orgName) {
        String assetType = assetCard.getAcBvName1();
        String prefix = orgName.contains("周宁县总医院") ? "周总" : "周县";
    ...
    private String getManufactureNo(AssetCardWithDepreciationDTO assetCard, String orgName) {
        ...
        if (assetType != null && assetType.contains("医疗专用设备") &&
                (orgName.contains("周宁县总医院") || orgName.contains("周宁县医院"))) {
    ...
    private String getInvoNo(AssetCardWithDepreciationDTO assetCard, String orgName) {
        ...
        if (assetType != null) {
            if (assetType.contains("网络信息设备") && orgName.contains("周宁县总医院")) {
