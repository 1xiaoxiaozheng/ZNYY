...
public class EquipRegInfoService_YL {
    ...
    public void syncEquipRegInfo() {
        ...
        try {
            // 获取seeyon中已删除的医疗器械信息
            List<AssetRegistrationMedicalQueryMapper.DeletedMedicalInfo> deletedMedicalInfoList = assetRegistrationMedicalQueryMapper
                    .selectDeletedMedicalInfo();

            if (!deletedMedicalInfoList.isEmpty()) {
                // 生成被删除的RID
                List<String> deletedRids = new ArrayList<>();
                for (AssetRegistrationMedicalQueryMapper.DeletedMedicalInfo deletedInfo : deletedMedicalInfoList) {
                ...
                if (!deletedRids.isEmpty()) {
                    ...
                    System.out.println("基于seeyon删除状态标记删除的医疗器械RID数量: " + deleteCount);
                    System.out.println("被删除的医疗器械RID: " + deletedRids);
                }
            }
        } catch (Exception e) {
