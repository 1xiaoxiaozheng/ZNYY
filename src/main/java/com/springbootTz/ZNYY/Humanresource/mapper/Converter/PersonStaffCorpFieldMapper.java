...
/**
 * 12. PersonStaffCorpFieldMapper - 人员学术社团任职信息
 * 数据说明：人员在学术社团中的任职信息
 * PostgreSQL表：ehr_org_person_detail_custom (detail_id =
 * "person_detail_VXVwV5ve")
 * Oracle表：HUM_STAFF_CORP
 * 同步方法：syncSocietyInfoAll()
 */
@Component
public class PersonStaffCorpFieldMapper {
    @Autowired
    ...
    public final Map<String, Function<PostgresPersonDetailCustom, String>> FIELD_MAPPING = new LinkedHashMap<String, Function<PostgresPersonDetailCustom, String>>() {
        {
            ...
            put("CORP_NAME", toSafeString(p -> {
                String v = jsonKeyValueTool.getValueByKey(p.getCustomFields(), "person_IjuurdO8");
                return v == null ? " " : v;
            }));
            ...
            put("AWARDS_DEMO", toSafeString(p -> {
                String v = jsonKeyValueTool.getValueByKey(p.getCustomFields(), "person_gKwCGLaX");
                return v == null ? " " : v;
            }));
            put("APRVNO", toSafeString(p -> {
                String v = jsonKeyValueTool.getValueByKey(p.getCustomFields(), "person_wvSprjV7");
                return v == null ? " " : v;
            }));
            put("EMP_PROFTECHTTL", toSafeString(p -> {
                String v = jsonKeyValueTool.getValueByKey(p.getCustomFields(), "person_Q43uo1Mn");
                return v == null ? " " : v;
            }));
    ...
    /**
     * 反射设置OracleStaffCorp属性（字段名需与OracleStaffCorp属性一致，支持下划线转驼峰）
     * String类型赋值为字符串，Date/BigDecimal类型空字符串赋null，日期自动识别格式
     */
    public void setOracleField(OracleStaffCorp oracle, String field, String value) {
